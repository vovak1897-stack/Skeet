...
local function CreateOptions(Frame)
    ...
    -- Вставьте этот код в раздел, где определена функция CreateOptions, замените stubbed Options.Dropdown на это.
        ...
        local Container = Utility.new("Frame", {
            ...
            Utility.new("ImageButton", {
                ...
                Utility.new("TextLabel", {
                    ...
                    Utility.new("UIPadding", {PaddingLeft = UDim.new(0, 5)})
                }),
                Utility.new("ImageLabel", {
                    Name = "Arrow",
                    AnchorPoint = Vector2.new(1, 0.5),
            ...
            Utility.new("Frame", {
                ...
                Utility.new("ScrollingFrame", {
                    ...
                    Utility.new("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5), PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5)})
                })
            })
        })

        local ListFrame = Container.ListContainer.List
        local Expanded = false
        local MaxHeight = 150  -- Максимальная высота списка, можно настроить

        local function UpdateList()
            ListFrame:ClearAllChildren()
            ListFrame.UIListLayout = Utility.new("UIListLayout", {
                SortOrder = Enum.SortOrder.LayoutOrder,
                Padding = UDim.new(0, 2)
            })
            ListFrame.UIPadding = Utility.new("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5), PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5)})

            local ContentHeight = 0
            for _, item in ipairs(Properties.List) do
                ...
                ItemButton.MouseButton1Click:Connect(function()
                    ...
                    Container.Selector.Current.Text = tostring(item)
                    local Success, Error = pcall(Properties.Function, item)
                    assert(Luminosity.Settings.Debug == false or Success, Error)
                    -- Закрыть список после выбора
                    Expanded = false
                    Utility.Tween(Container.ListContainer, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 0)}):Play()
                    Utility.Tween(Container.Selector.Arrow, TweenInfo.new(0.25), {Rotation = 0}):Play()
                    Utility.Tween(Container, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 25)}):Play()
                end)
                ItemButton.Parent = ListFrame
                ContentHeight = ContentHeight + 25 + 2  -- Высота элемента + padding
            end
            ContentHeight = ContentHeight - 2  -- Убрать лишний padding
            ListFrame.CanvasSize = UDim2.new(0, 0, 0, ContentHeight)
            return math.min(ContentHeight + 10, MaxHeight)  -- +10 для padding
        end

        local listHeight = UpdateList()

        Container.Selector.MouseButton1Click:Connect(function()
            Expanded = not Expanded
            if Expanded then
                Utility.Tween(Container.ListContainer, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, listHeight)}):Play()
                Utility.Tween(Container.Selector.Arrow, TweenInfo.new(0.25), {Rotation = 180}):Play()
                Utility.Tween(Container, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 25 + listHeight + 5)}):Play()
            else
                Utility.Tween(Container.ListContainer, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 0)}):Play()
                Utility.Tween(Container.Selector.Arrow, TweenInfo.new(0.25), {Rotation = 0}):Play()
                Utility.Tween(Container, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 25)}):Play()
        ...
        return setmetatable({}, {
            ...
            __newindex = function(Self, Index, Value)
                ...
                elseif Index == "List" then
                    ...
                    listHeight = UpdateList()
                elseif Index == "Selected" then
                    Properties.Selected = Value
                    Container.Selector.Current.Text = Value and tostring(Value) or Properties.Placeholder
                    local Success, Error = pcall(Properties.Function, Value)
                    assert(Luminosity.Settings.Debug == false or Success, Error)
                elseif Index == "Placeholder" then
                    Properties.Placeholder = Value and tostring(Value) or "Select"
                    if not Properties.Selected then
                        Container.Selector.Current.Text = Properties.Placeholder
                    end
                end
                Properties[Index] = Value
            end
